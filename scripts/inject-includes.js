...
class IncludeInjector {
  ...
  async injectIncludes() {
    console.log('Injecting includes...');
    ...
    console.log('Include injection complete!');
  }
  ...
  async processFileToOutput(sourcePath, outputPath) {
    ...
    while ((match = includeRegex.exec(content)) !== null) {
      ...
      const fullIncludePath = path.join(this.docsDir, includePath);
      ...
      if (fs.existsSync(fullIncludePath)) {
        ...
        const fullDetailedPath = path.join(this.docsDir, detailedPath);
...
if (require.main === module) {
  const injector = new IncludeInjector();
  injector.injectIncludes().catch(console.error);
}

module.exports = IncludeInjector;