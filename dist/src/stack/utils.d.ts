export declare function transform(type: any): () => any;
export declare function merge(target: {
    [key: string]: any;
}, source: {
    [key: string]: any;
}): {
    [key: string]: any;
};
export declare function mergeDeep(oldTarget: {
    [key: string]: any;
}, oldSource: {
    [key: string]: any;
}): {
    [key: string]: any;
};
export declare function _type(val: any): "string" | "number" | "bigint" | "boolean" | "symbol" | "undefined" | "object" | "function";
export declare function addParam(key: any, value: any): any;
export declare function addQuery(key: any, value: any): any;
export declare function language(languageCode: any): any;
export declare function environment(environmentCode: any): any;
export declare function includeOwner(): any;
export declare function includeContentType(): any;
export declare function includeSchema(): any;
export declare function includeReference(val: any): any;
export declare function getReferences(): any;
//# sourceMappingURL=utils.d.ts.map